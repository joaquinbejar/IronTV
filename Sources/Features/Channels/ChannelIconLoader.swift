...
final class ChannelIconLoader {
    ...
    /// ~500 decoded icons ≈ a few MB at typical panel icon sizes — enough
    /// that scrolling back through a long list is instant, small enough to
    /// be irrelevant next to the video buffers.
    ...
    func image(for url: URL) async -> CGImage? {
        ...
        let task = subscribe(url)
        return await withTaskCancellationHandler {
            await task.value
        } onCancel: {
            Task { @MainActor [weak self] in self?.unsubscribe(url) }
        }
    }

    private func subscribe(_ url: URL) -> Task<CGImage?, Never> {
        ...
        let task = Task { [weak self] () -> CGImage? in
            ...
            self?.finishLoad(of: url, with: image)
            return image
        }
        inFlight[url] = Entry(task: task, waiters: 1)
        return task
    }

    private func finishLoad(of url: URL, with image: CGImage?) {
        if let image {
    ...
    private func unsubscribe(_ url: URL) {
    ...
    /// ImageIO decode to an immutable `CGImage`; nil for anything that isn't
    /// an image. Runs detached — decoding is the CPU cost worth keeping off
    /// the main actor, not the network wait.
